...
//    }
    ...
    static func getSchemesForUser(owner: Profile, schemesDownloaded: [Scheme], notContainedStatus: [StatusScheme], pagination: Int, completionHandler: @escaping (_ success: Bool, _ msg: String, _ schemes: [Scheme]?) -> ()) {
        ...
        ParseRequest.queryEqualToValueNotContainedObjects(className: "Scheme", queryType: .or, params: queryParams, notContainedObjects: notContainedObjects, includes: ["requester", "owner", "post"], pagination: pagination) { (success, msg, objects) in
            if success {
                for obj in objects! {
